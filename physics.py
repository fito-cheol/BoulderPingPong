...
class Physics:
    ...
    def update(self, player_positions, dt):
        ...
        isTouchRight = self.ball_pos[0] > 1
        isTouchBottom = self.ball_pos[1] < config.BALL_RADIUS_RATIO
        isTouchTop = self.ball_pos[1] > 1 - config.BALL_RADIUS_RATIO
        ...
        if isTouchLeft:
            ...
            print('isTouchBottom or isTouchTop', self.ball_pos)
            self.ball_vel[1] = -self.ball_vel[1]

        if self.ignore_collisions:
            if (self.target_side == 'left' and self.ball_pos[0] < MAX_SCREEN / 2) or \
