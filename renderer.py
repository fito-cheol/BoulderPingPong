...
import numpy as np
import cv2
from config import WALL_WIDTH, WALL_HEIGHT, SCREEN_WIDTH, SCREEN_HEIGHT, BALL_RADIUS, HITBOX_RADIUS, SCALE_FACTOR, COLORS, FOCUS_X, FOCUS_Y
...
class Renderer:
    ...
    def transform_coordinates(self, points):
        ...
        points = points - np.array([FOCUS_X, FOCUS_Y])  # 초점 오프셋 적용
        transformed = cv2.perspectiveTransform(points, self.homography)
        scale = np.array([SCREEN_WIDTH / WALL_WIDTH, SCREEN_HEIGHT / WALL_HEIGHT])
    ...
    def render(self, ball_pos, player_positions, score):
        ...
        for pos in player_positions:
            ...
            color = COLORS['hand'] if pos in player_positions[:2] else COLORS['foot']
